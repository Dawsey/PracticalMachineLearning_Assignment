...
2. Random Forest using the `caret` package
3. Random Forest using the `randomForest` package
4. Linear Discriminant Analysis using the `caret` package.

Of the four modelling techniques, the Random Forrest approach provided the highest accuracy and was used for the final graded assessment. The random forest model developed with the `randomForest` package performed the best overall.
...
df.metrics.lda <- data.frame(Model = "Linear Discriminant Analysis", Accuracy = round(pred.lda.cm$overall['Accuracy'], 3))
```
...
df.metrics.gbm <- data.frame(Model = "Gradient Boosting", Accuracy = round(pred.gbm.cm$overall['Accuracy'], 3))
...
```{r rfcModel}
set.seed(825)
mdl.rfc <- train(classe ~ ., data = Train, 
                 method = "rf", 
                 trControl = fitControl,
                 verbose = FALSE)
mdl.rfc 
...
#### Cross Validation
```{r}
pred.rfc <- predict(mdl.rfc, Test)

pred.rfc.cm <- confusionMatrix(pred.rfc, Test$classe)

df.metrics.rfc <- data.frame(Model = "Random Forest - CARET", Accuracy = round(pred.rfc.cm$overall['Accuracy'], 3))
```
...
df.metrics.rand <- data.frame(Model = "Random Forest", Accuracy = round(pred.randForr.cm$overall['Accuracy'], 3))
```

## Compare Models
The cross validated accuracies of each of the models is compared in the following table to provide the out of sample error:

```{r Comparison}
df.comparison <- rbind(df.metrics.gbm, df.metrics.rfc, df.metrics.rand, df.metrics.lda)
row.names(df.comparison) <- NULL
kable(df.comparison %>%
  arrange(desc(Accuracy)))
```

Based on the above table the Random Forrest model from the `randomForest` package yeilds the highest accuracy and will be used in the final predictions for the quiz.
```{r}
